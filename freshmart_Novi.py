...
<style>
    ...
    [data-testid="stSidebar"] div[role="radiogroup"] label[data-checked="true"] {
        background: linear-gradient(90deg, rgba(239, 68, 68, 0.25) 0%, rgba(185, 28, 28, 0.15) 100%) !important;
    ...
    [data-testid="stMetric"] {
        ...
        box-shadow: 0 10px 25px -5px rgba(185, 28, 28, 0.1) !important;
...
# ==========================================
    ...
    st.sidebar.markdown("<h2 style='background:linear-gradient(45deg, #ef4444, #f87171); -webkit-background-clip:text; -webkit-text-fill-color:transparent; font-size:22px; font-weight:800; margin-bottom:5px; margin-top:15px;'>FreshMart UI v2.5</h2>", unsafe_allow_html=True)
    ...
    if menu == "📢 1. Panggil Nomor Antrean":
        ...
        if antrean.is_empty():
            ...
            st.markdown(f"""
            <div class="clean-box">
                <p style="margin:0; font-size:12px; color:#b91c1c; font-weight:bold; text-transform:uppercase;">Sedang Melakukan Scan Barang</p>
    ...
    # --- MENU 5 ---
        ...
        if antrean.is_empty():
            ...
            st.markdown(f"<h3 style='background:none !important; -webkit-text-fill-color:#0f172a !important; color:#0f172a !important; margin-bottom:15px;'>Pembayaran Atas Nama: <span style='color:#b91c1c; -webkit-text-fill-color:#b91c1c !important;'>{pelanggan_depan.nama}</span></h3>", unsafe_allow_html=True)
            ...
            st.markdown(f"<h2 style='background:none !important; -webkit-text-fill-color:#0f172a !important; color:#0f172a !important;'>Total Tagihan: <span style='color:#b91c1c; -webkit-text-fill-color:#b91c1c !important;'>Rp {pelanggan_depan.total_harga:,}</span></h2>", unsafe_allow_html=True)
            ...
            if st.button("Finalisasi Pembayaran & Cetak Nota", type="primary", use_container_width=True):
                if uang_bayar < pelanggan_depan.total_harga:
                    ...
                    <div class="struk-container">
                        ...
                        <div class="struk-item-row" style="color:#dc2626; font-weight:bold;">
    ...
    # --- MENU 6 ---
        ...
        st.markdown(f"""
        <div style="background: linear-gradient(90deg, #0f172a 0%, #b91c1c 100%); padding: 18px 25px; border-radius: 16px; margin-bottom: 25px;">
            <div style="font-size: 11px; font-weight: 700; color: #fee2e2; text-transform: uppercase;">LIVE FINANCIAL TRACKER</div>
            <div style="font-size: 28px; font-weight: 800; color: #ffffff; margin-top: 2px;">Total Arus Pendapatan: <span style="color: #f87171;">Rp {st.session_state.total_omset:,}</span></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_jurnal:
            ...
            if not st.session_state.riwayat_transaksi:
                ...
                for r in st.session_state.riwayat_transaksi:
                    st.markdown(f"<p style='color:#b91c1c; font-weight:600;'>✓ {r}</p>", unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
